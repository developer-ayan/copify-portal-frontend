...
const Navbar = ({ toggle, setToggle }) => {
  ...
  const filteredNavLinks = (userRole) => {
    if (user?.role_id == 1) {
      ...
      return navLinks.filter((link) =>
        [1, 2, 4, 5, 6, 8].includes(link.id)
      );
    } else if (user?.role_id == 2) {
      // Show these items when user role is 2
      return navLinks.filter((link) =>
        [3, 7].includes(link.id)
      );
    } else {
  ...
  return (
    <>
      <nav
        id="navbar"
        className={`flex flex-col justify-between h-screen overflow-y-auto absolute md:static top-0 left-0 border-r bg-white ${toggle ? "" : "-translate-x-full md:-translate-x-0"
  ...
  if (data.path) {
    return (
      <NavLink
        to={data.path}
        className={({ isActive }) => {
          return `${isActive ? "text-blue-500 font-semibold" : "text-[#091A35]"
      ...
      <div className={`${toggle ? "block" : "hidden"} relative ml-7 text-xs`}>
        ...
        {data.items.map(({ path, title }) => {
          return (
            ...
            >
              <div
                className={`${window.location.pathname === path
                  ? "bg-[#909090] scale-110"
                  : "bg-[#D9D9D9]"
