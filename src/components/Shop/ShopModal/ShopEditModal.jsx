...
const ShopEditModal = ({ show, onClose, onSave, currentInstitute, isLoading }) => {
  const [name, setName] = useState(currentInstitute?.shop_name || '');
  const [address, setAddress] = useState(currentInstitute?.shop_address || '');

  const handleSave = () => {
    onSave(currentInstitute.shop_name, name, address);
    onClose();
  };
      ...
      <div className="bg-white w-full max-w-md mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-4 overflow-y-auto max-h-screen">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-xl font-semibold text-gray-800">Edit Shop</h2>
