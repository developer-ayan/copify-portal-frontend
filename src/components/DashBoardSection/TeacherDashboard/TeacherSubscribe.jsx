...
const TeacherSubscribe = ({ item }) => {
  ...
  const getSubscibers = async (value) => {
    try {
      ...
      const response = await call('/admin/fetch_teacher_subscriber_list', 'POST', formData)
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr>
                    ...
                    <th className="px-4 py-2 border text-center">Course & Major</th>
                ...
                <tbody>
                  {uploads.map((upload, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-4 py-2 border text-center">{getValueById(subjectList, subject, 'id')}</td>
                      <td className="px-4 py-2 border text-center">{upload.year}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
