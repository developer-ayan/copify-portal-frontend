      ...
      <div className="bg-white p-4 rounded shadow">
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          <div className="flex-[2]">
            ...
            />
            <button className=" bg-blue-500 text-white px-4 py-2 rounded mt-4 ml-4 ">CLEAR</button>
            <div className="space-y-4 mt-4">
              ...
              {[
                ...
                <div className="flex items-center text-center" key={idx}>
                  <label className="block mb-1 w-1/2">{field.label}:</label>
                  <input
                    type="text"
                    name={field.name}
                    value={formData[field.name]}
                    onChange={handleInputChange}
                    className="w-1/4 border rounded text-center"
                  />
                </div>
              ))}
              ...
              {[
                ...
                <div className="flex items-center text-center" key={idx}>
                  ...
                  <input
                    ...
                    value={formData[field.name]}
                    onChange={handleInputChange}
                    className="w-1/4 border text-center right"
                  />
                </div>
                 ))}
              ...
              {[ 'totalPayment', 'totalWalletPaid', 'folderNumber', 'paymentStatus'].map((field, idx) => (
                <div className="flex items-center" key={idx}>
                  <label className="block mb-1 w-1/2 capitalize text-lg font-semibold">{field.replace(/([A-Z])/g, ' $1')}:</label>
                  <input
                    type="text"
                    name={field}
